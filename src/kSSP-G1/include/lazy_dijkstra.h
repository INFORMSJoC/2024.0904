...
public:
    ...
    // Get Dijkstra rank of u (number of extractions from heap needed
    // to find the shortest path to target)
    // Warning: not properly maintained for copy constructor disabling vertices or after a vertex has been enabled.
    TI rank(const TI &u);
...
graph(other->graph), n(other->n), target(other->target)
{
    ...
    std::memcpy(_rank, other->_rank, sizeof(TI) * n);
...
/*
 * Get Dijkstra rank of vertex u, i.e., number of extractions from heap
 * until the shortest path from u to target has been found
 */
template<typename TI, typename TV>
inline TI LazyDijkstra<TI, TV>::rank(const TI &u)
{
    // If u is not seen, we compute its path.
    // If the top of the heap has a small weight, an update has been done and might change this weight
    if (not seen[u])
