...
private:
    ...
    kssp::PostponedNodeClassification<DGTI, TV> *PNC;
    kssp::PostponedNodeClassificationStar<DGTI, TV> *PNCs;
    ...
    */
    ...
    while (!file.eof()) {
        if ((c == 'c') or (c == 'p')) {
            ...
            TI source_id, target_id;
            TV weight;
            file >> source_id >> target_id >> weight;
            assert((source_id >= 0) && (target_id >= 0));
        ...
        file >> c; // read first char of the line, or set eof flag to true
    }
    file.close();
...
{
  if (Y != nullptr) delete Y;
  if (PNC != nullptr) delete PNC;
  if (NC != nullptr) delete NC;
  if (PNCs != nullptr) delete PNCs;
                        ...
                        size_t version, bool verbose)
{
  ...
  else if ((algorithm.compare("PNC") == 0) || (algorithm.compare("Postponed_Node_Classification") == 0))
    PNC = new kssp::PostponedNodeClassification<DGTI, TV>(g, vertex_to_int[source], vertex_to_int[target]);
  else if ((algorithm.compare("Y") == 0) || (algorithm.compare("Yen") == 0))
    Y = new kssp::Yen<DGTI, TV>(g, vertex_to_int[source], vertex_to_int[target]);
  else if ((algorithm.compare("PNC*") == 0) || (algorithm.compare("Postponed_Node_Classification_Star") == 0))
        PNCs = new kssp::PostponedNodeClassificationStar<DGTI, TV>(g, vertex_to_int[source], vertex_to_int[target]);
  else
...
std::pair<std::vector<TI>, TV> EasyDirectedGraph<TI, TV, DGTI>::next_path()
{
  ...
  return std::make_pair(convert_path(res.first), res.second);
    ...
    {
auto res = PNC->next_path();
return std::make_pair(convert_path(res.first), res.second);
    }
  else if (PNCs != nullptr)
    {
auto res = PNCs->next_path();
return std::make_pair(convert_path(res.first), res.second);
    }
...
size_t EasyDirectedGraph<TI, TV, DGTI>::used_trees()
{
  if (PSB != nullptr)
    ...
    return PNC->cpt_used_trees;
  else if (PNCs != nullptr)
    return PNCs->cpt_used_trees;
  else
    return 0;
}
