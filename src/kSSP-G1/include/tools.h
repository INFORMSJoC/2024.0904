...
namespace tools_for_kssp {
  ...
  public:
    ...
    TV weight;
    size_t dev_idx;
    bool simple; // used for PNC and PNCs

    CandidatePath() = default;

    // Constructor
    explicit CandidatePath(std::vector<TI> path, const TV weight, const size_t deviation_index);
      // Constructor/4 for PNC and PNCs
      explicit CandidatePath(std::vector<TI> path, const TV weight, const size_t deviation_index, bool is_simple);
...
// Constructor/4 for PNC and PY

  template<typename TI, typename TV>
CandidatePath<TI, TV>::CandidatePath(std::vector<TI> ppath, const TV weight, const size_t deviation_index, bool is_simple):
  path(ppath), weight(weight), dev_idx(deviation_index), simple(is_simple)
{
