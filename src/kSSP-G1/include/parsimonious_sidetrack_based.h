...
private:
    ...
    // v2
    void treat_one_hard_candidate_v2();

    // v3
    void treat_one_hard_candidate_v3();
...
std::pair<std::vector<TI>,TV> ParsimoniousSidetrackBased<TI, TV>::next_path()
{
    ...
    while (not empty())
    {
        ...
        {
            if (version == 2)
                treat_one_hard_candidate_v2();
            else if (version == 3)
                treat_one_hard_candidate_v3();
            else
 ...
 * method performs a node classification ?la Feng. That is, it assigns colors
...
/*
* Update the value of the threshold (theta)
*
* The value of the threshold is dynamic. i.e, it changes during the execution of the program.
* A slight change here, the value of the threshold is the ratio between
* the least simple element and the least no simple one times a constant. This is easier than computing
* The pair of strictly least elements as it saves an extraction and re-insertion of an element in a the heap
*
*/
template<typename TI, typename TV>
void ParsimoniousSidetrackBased<TI, TV>::update_threshold(){
    if (heap_hard_candidate_paths->empty() or heap_candidate_paths->empty())
        return;
    // roughly, the value of the threshold is the rato between the two coming paths + small constant
    float curr_threshold = heap_candidate_paths->top_item()->weight*1.0/ heap_hard_candidate_paths->top_item()->weight;
    if(curr_threshold < 1)
        curr_threshold = 1.0/curr_threshold; // a threshod should be bigger than 1
    float C = 10*(curr_threshold - 1);
    curr_threshold += C;
    // std::cout<<"curr_threshold "<<curr_threshold<<"  threshold"<<threshold<<std::endl;
...
void ParsimoniousSidetrackBased<TI, TV>::treat_one_hard_candidate()
{
    ...
    { // We are done with this hard candidate
        ...
        { // This tree has never been used, so we can delete it to save memory
            delete T_hard;// We may do: cpt_used_trees--; however, we don't. So, cpt_used_trees is an upper bound on the number of stored trees
...
/* Version 2 : here we store the tree ot T_min if and only if it leads to an interesting path : path with weight less than theta*weight of last path */

template<typename TI, typename TV>
void ParsimoniousSidetrackBased<TI, TV>::treat_one_hard_candidate_v2()
{
    if (heap_hard_candidate_paths->empty())
        return;  // should not happen
    ...
    // we update the value of the threshold
    update_threshold();
    
    // the main loop iterating over the bad sidtracks: sidetreacks leading to no simple paths
    while ((not hard_candidate->sidetracks.empty()) &&
           (hard_candidate->sidetracks.back().first >= hard_candidate->idx_min))
    {
        size_t idx = hard_candidate->sidetracks.back().first;
        ...
        if (T_hard->successor(head) != head)
        {
            ...
            new_candidate->dev_idx = idx + 1;
            new_candidate->sidetrack_sequence.push_back(std::make_pair(u, head));
            if (u == u_min and not heap_candidate_paths->empty() and new_candidate->weight <= threshold * heap_candidate_paths->top_item()->weight)
            { // We use the current working tree
                new_candidate->tree_index_sequence.push_back(hard_candidate->tree_index);
                T_hard_used = true;
            ...
            // Inserted to the (simple) heap with their real weight
            heap_candidate_paths->push(new_candidate, new_candidate->weight);
        }
        
        // Should we update the current working tree ?
        if (not T_hard_used and not hard_candidate->sidetracks.empty()) // There is more deviatins, and T_{u_min} is not used yet
        {
            size_t next_idx = hard_candidate->sidetracks.back().first;
            if (next_idx != idx)
            {
    ...
    { // update the hard candidate and push it back to heap
        ...
        if (T_hard_used) // Only if the tree is interesting we keep it, i.e, we creat new one
        { // We will need a new tree
            hard_candidate->tree_index = MAX_TREE_IDX;
        }
        heap_hard_candidate_paths->push(hard_candidate, lbw);
...
/* Version 3 : here we store all of the interesting trees : trees leading to an interesting path : a path with weight less than theta * weight of last path */


template<typename TI, typename TV>
void ParsimoniousSidetrackBased<TI, TV>::treat_one_hard_candidate_v3()
{
    if (heap_hard_candidate_paths->empty())
        return;  // should not happen
    ...
    dijkstra::LazyDijkstra<TI,TV> *T_orig = idx_to_tree[hard_candidate->candidate.tree_index_sequence.back()];
    dijkstra::LazyDijkstra<TI,TV> *T_hard, *T_temp;
    if (hard_candidate->tree_index == MAX_TREE_IDX)
    { // We need a new (working) tree
           ...
           (hard_candidate->sidetracks.back().first >= hard_candidate->idx_min))
    {
        ...
        if (T_hard->successor(head) != head)
        {
            ...
            if (u == last_spur)
            { // we reuse the last tree (same index)
                if (not T_saved and new_candidate->weight <= threshold * heap_candidate_paths->top_item()->weight)
                {
                    // current_tree is not saved but it should be saved now sincie it leads to an interesting path
                    T_temp = new dijkstra::LazyDijkstra<TI,TV>(T_hard, &prefix);
                    new_candidate->tree_index_sequence.push_back(idx_to_tree.size());
                    idx_to_tree.push_back(T_temp);
                    cpt_used_trees++;
                    T_saved = true; // So we don't duplicate trees
                }
                else
                 new_candidate->tree_index_sequence.push_back(current_tree_index);
            }
            else
            {
                ...
                // since the tree is modified
                T_saved = false;
                // We "create" a new tree
                T_temp = nullptr;
                
                if (not heap_candidate_paths->empty() and new_candidate->weight <= threshold * heap_candidate_paths->top_item()->weight)
                {
                    T_temp = new dijkstra::LazyDijkstra<TI,TV>(T_hard, &prefix);
                    T_saved = true;
                    cpt_used_trees++;
                ...
                current_tree_index = idx_to_tree.size();
                new_candidate->tree_index_sequence.push_back(current_tree_index);
                idx_to_tree.push_back(T_temp); // nullptr if no T_temp
                last_spur = u;
        ...
        // Should we update the current working tree ?
        if (not hard_candidate->sidetracks.empty()) // There is more deviations, and T_{u_min} is not used yet
        {
            size_t next_idx = hard_candidate->sidetracks.back().first;
            if (next_idx != idx)
            {
