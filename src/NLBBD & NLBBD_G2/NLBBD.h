...
void NLBBD(Instance ins, std::string const& ins_name, std::string const& txtfile,
    ...
    bool& addCut1, bool& addCut2, bool& addCut3, bool& LiftCut2, bool& LiftCut3, int& sp2_method, bool& use_Heu, double time_limit)
{
    ...
    for (int i = 0; i < ins.d_T; i++)
        for (int b = 0; b < ins.num_b; b++)
            for (int m = 0; m < ins.num_m; m++)
            {
                std::string var_name = "x[" + std::to_string(i + 1) + "]["
                    + std::to_string(b + 1) + "][" + std::to_string(m + 1) + "]";
                GRBVar x = MP_model.addVar(0.0, 1.0, 0.0, GRB_BINARY, var_name);
                X[std::make_tuple(i + 1, b + 1, m + 1)] = x;
            }
    ...
    {
        std::string var_name = "f[" + std::to_string(i + 1) + "][" + std::to_string(i + 2) + "]";
        GRBVar f = MP_model.addVar(0.0, 1.0, 0.0, GRB_CONTINUOUS, var_name);
        F[std::make_tuple(i + 1, i + 2)] = f;
    }
    ...
    for (int i = 0; i < ins.d_T - 1; i++)
    {
        std::string var_name = "g[" + std::to_string(i + 1) + "][" + std::to_string(i + 2) + "]";
        GRBVar g = MP_model.addVar(0.0, 1.0, 0.0, GRB_CONTINUOUS, var_name);
        G[std::make_tuple(i + 1, i + 2)] = g;
    }
    ...
    {
        GRBLinExpr con1 = 0;
        for (int b = 1; b <= ins.num_b; b++)
            for (int m = 1; m <= ins.num_m; m++)
    ...
    while (lbbdInfo->UB - lbbdInfo->LB > 1e-3)
    {
        ...
        std::cout << "Finish solving SP." << std::endl;
    ...
    {
        std::cout << "Do not find the optimal solution within the timeLimit." << std::endl;

        out_txt_f << "Do not find the optimal solution within the timeLimit." << std::endl;
