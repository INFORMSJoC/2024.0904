...
verbose(verbose), graph(g), source(ssource), target(ttarget), version(version)
{
    ...
    CandidateSimple<TI,TV> *candidate = new CandidateSimple<TI,TV>(0, // lower bound on weight of the path
                                                                   0, // deviation index
                                                                   sidetrack_sequence,
                                                                   tree_index_sequence);
    heap_candidate_paths->push(candidate, 0);
}
...
void SidetrackBased<TI,TV>::extract_path(CandidateSimple<TI,TV> *candidate_path)
{
    ...
    TI u = extracted_path.back();
    if ((u != target) and (u == idx_to_tree[current_tree]->successor(u))) {
        // We have no path to target. This may happen if the graph is not connected
        extracted_path.clear();
        extracted_path_weight = 0;
    } else {
        extracted_path_weight += idx_to_tree[current_tree]->weight(u);
