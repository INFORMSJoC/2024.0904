 ...
 * This method is a special version of the postponed NC. The main intuition is to speed up the cumputation
 * of a shortest simple detour using the classification of Feng [1]
 *
...
#ifndef POSTPONED_NODE_CLASSIFICATION_STAR_H
#define POSTPONED_NODE_CLASSIFICATION_STAR_H
...
{
public:
    PostponedNodeClassificationStar() = default;

    // Constructor
    explicit PostponedNodeClassificationStar(directed_graph::DirectedGraph<TI,TV> *g,
                                             const TI ssource,
                                             const TI ttarget);

    // Destructor
    virtual ~PostponedNodeClassificationStar();
...
PostponedNodeClassificationStar<TI, TV>::PostponedNodeClassificationStar(directed_graph::DirectedGraph<TI,TV> *g,
...
// Destructor
template<typename TI, typename TV>
PostponedNodeClassificationStar<TI, TV>::~PostponedNodeClassificationStar()
{
    if (color != nullptr)
    {
        free(color);
...
// Initialize data structures for the manipulation of colors and residual weights
template<typename TI, typename TV>
void PostponedNodeClassificationStar<TI, TV>::init_data_structures()
{
...
// Check whether there is a candidate path
template<typename TI, typename TV>
inline bool PostponedNodeClassificationStar<TI, TV>::empty()
{
...
// Get next path
template<typename TI, typename TV>
std::pair<std::vector<TI>,TV> PostponedNodeClassificationStar<TI, TV>::next_path()
{
    std::vector<TI> path;
    path.clear();
...
// Compute and store deviations of this path
template<typename TI, typename TV>
void PostponedNodeClassificationStar<TI, TV>::compute_deviations(tools_for_kssp::CandidatePath<TI,TV> *candidate_path)
{
...
// Compute and store deviations of this path
template<typename TI, typename TV>
void PostponedNodeClassificationStar<TI, TV>::repair_path(tools_for_kssp::CandidatePath<TI,TV> *candidate_path)
{
    ...
    //for (size_t i = candidate_path->dev_idx; i <= candidate_path->dev_idx; i++)
    //  stack.push_back(candidate_path->path[i]);
...
// Special dijsktra from spur to green node or target
template<typename TI, typename TV>
void PostponedNodeClassificationStar<TI, TV>::special_dijkstra(const TI spur_node)
{
    for (size_t u = 0; u < n; u++)
    {
        weight[u] = MAX_WEIGHT;
