    ...
    void callback()
    {
        try
        {
            if (where == GRB_CB_MIPSOL)
            {
                ...
                // addLazyCut1(Ins, down_seq, vars, OptSPInformation);
    ...
    int sp2_method, double sp_time_limit)
{
    ...
    std::cout << "remain time: " << std::to_string(sp_time_limit) << std::endl;
    if (sp_time_limit < 1e-4)
    {
        std::cout << "The remaining time is not enough to solve the SP." << std::endl;
